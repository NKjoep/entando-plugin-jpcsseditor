...
public class ResourceeditorAction extends BaseAction implements IResourceeditorAction {
	...
	public String list () {
		...
		try {
			fold = this.getJpcsseditorManager().getCssMap(root,name,searchSubfolders);
		...
		if (listPlugins) {
			try {
				TreeMap<String, ArrayList<String>> pluginsList = this.getJpcsseditorManager().getCssMap(this.getRootFolder()+"plugins/", name, true);
	...
	public String edit () {
		...
		String cssContent;
		try {
			cssContent = this.getJpcsseditorManager().readCss(filePath);
	...
	public String save() {
		...
		if (strutsAction==1) {
			...
			if (strutsAction==2){
				...
				try {
					this.getJpcsseditorManager().writeCss(filePath, fileContent);
	...
	public String delete() {
		...
		if (fileToDelete!=null && fileToDelete.trim().length()>0) {
			...
			try {
				this.getJpcsseditorManager().delete(fileToDelete);
				return SUCCESS;
			}
			catch (ApsException e) {
	...
	public String saveNew() {
		...
		if (folder!=null && folder.trim().length()>0 && name!=null && name.trim().length()>0) {
			...
			name = name.replaceAll("\\.\\./","");
			try {
				File newFile = this.getJpcsseditorManager().create(folder, name);
				this.getJpcsseditorManager().writeCss(newFile.getAbsolutePath(), content);
				return SUCCESS;
			}
			catch (ApsException e) {
	...
	public List<String> getCssFoldersMap() {
		...
		try {
			staticList = this.getJpcsseditorManager().getCssFoldersList(rootFolder+"static/css");
			pluginlist = this.getJpcsseditorManager().getCssFoldersList(rootFolder+"plugins");
		...
		} catch (ApsException e) { System.out.println("jpcsseditor: "+ e ); }
		if (map.size()==0) {
			map.add("static/css");
		}
		return map;
	...
	public ICssManager getJpcsseditorManager() {
		return _jpcsseditorManager;
	}

	public void setJpcsseditorManager(ICssManager jpcsseditorManager) {
		this._jpcsseditorManager = jpcsseditorManager;
	}

	private ICssManager _jpcsseditorManager;
